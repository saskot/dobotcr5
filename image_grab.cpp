	...
	if (sensorHandle)
	{
		...
		if (Sensor3D_GetSensorStatus(sensorHandle, &sensorHandlestatus) == SENSOR3D_OK)
		{
			if (sensorHandlestatus & SENSOR_CONNECTED)
			{
				...
				//Set SensorMode 5 The sensor should be in live image mode, see command SetSensorMode=5
				command = "SetSensorMode=5\r";
				...
				//Set Pattern to 28Patterns (highest Precision)x=solid: Used for 2D camera images (SetSensorMode=5). The user should switch the
				//LED pattern to solid in 2D camera image
                command = "SetLEDPattern=solid\r";
				result = Sensor3D_WriteData(sensorHandle, command.data());
				if (result != SENSOR3D_OK)
				{
					std::cerr << "Error, could not write " << command << ", result = " << result << std::endl;
				...
				for (int i = 0; i < nrScans; i++) {
					...
					result = Sensor3D_GetCameraImage(sensorHandle, &scanBuffer, pc_size, &roi, 3000);
					if (result != SENSOR3D_OK)
					{
						std::cerr << "Error, could not read Pointcloud, result = " << result << std::endl;
					}
